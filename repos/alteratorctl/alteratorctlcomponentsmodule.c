...
// Output list delimiters:
#define COMPONENTS_MODULE_TREE_TOP "┌"
#define COMPONENTS_MODULE_TREE_VLINE "│"
#define COMPONENTS_MODULE_TREE_HLINE "─"
#define COMPONENTS_MODULE_TREE_BRANCH "├"
#define COMPONENTS_MODULE_TREE_END "└"
...
int components_module_run(gpointer self, gpointer data)
{
    ...
    case COMPONENTS_INSTALL:
    	if (!(locale = alterator_ctl_get_locale()))
            ERR_EXIT();
    ...
    if (!ignore_sections)
    {
        ...
        for (int section_index = 0; section_index < amount_of_sections; section_index++)
        {
            ...
            components_module_print_list_with_filters(module,
                                                      ...
                                                      section_name,
                                                      section_index == 0,
                                                      section_index == (amount_of_sections - 1));
                                                                   ...
                                                                   GString *buffer)
{
    ...
    if (is_first)
        connector = COMPONENTS_MODULE_TREE_TOP;
    else if (is_last && !node->parent->parent && !is_first)
        connector = COMPONENTS_MODULE_TREE_BRANCH;
    else if (is_last)
        connector = COMPONENTS_MODULE_TREE_END;
    else
        connector = COMPONENTS_MODULE_TREE_BRANCH;
    ...
    while (child)
    {
        ...
        if (!node->parent->parent && (is_single_node || !is_first))
            tree_line = COMPONENTS_MODULE_TREE_VLINE;
        else if (is_last)
            tree_line = " ";
        else
            tree_line = COMPONENTS_MODULE_TREE_VLINE;
                                              ...
                                              gchar **result)
{
    ...
    if (!info_parser->alterator_ctl_module_info_parser_find_table(
            info_parser, elem_data, &display_name, COMPONENT_ALTERATOR_ENTRY_COMPONENT_DISPLAY_NAME_TABLE_NAME, NULL))
    {
