...
static const gchar xml_param_in_name[] =
  "      <arg type='s' name='";

static const gchar xml_param_in_name_array[] =
  "      <arg type='as' name='";

static const gchar xml_param_in_name_end[] =
  "' direction='in'/>";

static const gchar xml_method_output_strings[] =
  "      <arg type='as' name='stdout_strings' direction='out'/>";

static const gchar xml_method_output_bytes[] =
  "      <arg type='ay' name='stdout_bytes' direction='out'/>";

static const gchar xml_method_output_byte_arrays[] =
  "      <arg type='aay' name='stdout_byte_arrays' direction='out'/>";

static const gchar xml_method_output_string_array[] =
  "      <arg type='as' name='stdout_string_array' direction='out'/>";

static const gchar xml_method_output_string[] =
  "      <arg type='s' name='";

static const gchar xml_method_output_string_end[] =
  "' direction='out'/>";

static const gchar xml_method_error_strings[] =
...
static gboolean on_child_exited(GPid pid, gint wait_status, gpointer data) {
    ...
    //for glib2 version >= 2.70
    //gboolean result = g_spawn_check_wait_status(wait_status, &error);
...
/* Validates a valid name for a d-bus method or parameter. The name can only
   ...
   the name cannot be greater than 100 and less than 2. */
...
static gboolean is_correct_execute(gchar **execute) {
    ...
    while (*execute != NULL) {
        ...
        if (counter % 2 == 0) {
            ...
            if (g_str_has_suffix(*execute, "[]")) {
                gchar *array_name = g_strndup(*execute,
                                              g_utf8_strlen(*execute, -1) - 2);
                gboolean res = is_correct_name(array_name);
                g_free(array_name);
                if (!res) {
                    return FALSE;
                }
            } else if (!is_correct_name(*execute)) {
                return FALSE;
            }
        }
...
static void introspect_add_string_args(GString *xml, GStrv arg_names) {
    while (arg_names && *arg_names) {
        g_string_append(xml, xml_method_output_string);
        g_string_append(xml, *arg_names);
        g_string_append(xml, xml_method_output_string_end);
        arg_names++;
    }
}
                                            ...
                                            InterfaceObjectInfo *info)
{
                                   ...
                                   (gpointer *) &method_object_info))
    {
        ...
        while (*execute_data != NULL) {
            if (counter % 2 == 0) {
                ...
                if (g_str_has_suffix(*execute_data, "[]")) {
                    gchar *array_name = g_strndup(*execute_data,
                                          g_utf8_strlen(*execute_data, -1) - 2);
                    g_string_append(xml, xml_param_in_name_array);
                    g_string_append(xml, array_name);
                    g_free(array_name);
                } else {
                    g_string_append(xml, xml_param_in_name);
                    g_string_append(xml, *execute_data);
                }
                g_string_append(xml, xml_param_in_name_end);
            }
            counter++;
            execute_data++;
        }
        ...
        if (method_info->stdout_json_enabled) {
            introspect_add_string_args(xml, method_info->stdout_param_names);
        } else if (method_info->stdout_string_array_enabled) {
            g_string_append(xml, xml_method_output_string_array);
        } else if (method_info->stdout_byte_arrays_enabled) {
            g_string_append(xml, xml_method_output_byte_arrays);
        } else if (method_info->stdout_bytes_enabled) {
            g_string_append(xml, xml_method_output_bytes);
        } else if (method_info->stdout_strings_enabled) {
            g_string_append(xml, xml_method_output_strings);
        }

        if (method_info->stderr_strings_enabled) {
            g_string_append(xml, xml_method_error_strings);
        }
