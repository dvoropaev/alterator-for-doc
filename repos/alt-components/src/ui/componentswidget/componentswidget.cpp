...
void ComponentsWidget::setupTreeCustomContextMenu()
{
    ...
    connect(collapseAllAction.get(), &QAction::triggered, [&]() {
        ...
        if (controller->getViewMode() == MainWindow::ViewMode::BySections
            || controller->getViewMode() == MainWindow::ViewMode::ByTags)
        {
            expandTopLevel();
        }
    });
...
void ComponentsWidget::setController(Controller *controller)
{
...
{
    controller->setTextFilter(query);
}
...
void ComponentsWidget::changeEvent(QEvent *event)
{
    if (event->type() == QEvent::LanguageChange)
    {
        ...
        {
            controller->selectObject(selectedIndexes.at(0));
        }
        else
        {
            setDefaultDescription();
