    ...
    , warningsModel(std::make_unique<QStandardItemModel>(0, 2))
{
    ...
    mainWindow = std::make_unique<MainWindow>(this, this->model.get(), this->proxyModel.get());
...
void Controller::init()
{
    ...
    checkDate(DBusProxy::get().getDateResult(APT1_LAST_UPDATE_METHOD_NAME),
              0,
              tr("It seems your apt package lists weren't updated since %1"));
...
void Controller::changeLocale(const QLocale &locale)
{
    ...
    this->mainWindow->setupTranslator();
    retranslateWarningsModel();
}

void Controller::checkDate(std::optional<QDate> backendResult, int interval, const QString &warningMessage)
{
