...
#include "ui/misc/ObjectInfoDelegate.h"

#include "ui/misc/CustomTreeView.h"
...
public:
    inline explicit EditModel(const Action& action)
        : m_action{action}
    {
    ...
    int getRow(Property::Value* value) {
        ...
        return indexOf(static_cast<Parameter*>(value->property()));
    }
    ...
    Qt::ItemFlags flags(const QModelIndex& index) const override {
        ...
        flags.setFlag(Qt::ItemIsUserCheckable, index.column() == 0 && !required &&
                      ( !parent.isValid() ||
                        ...
                        parent.data(Qt::CheckStateRole).value<Qt::CheckState>() != Qt::Unchecked ) );
        return flags;
    }

    bool setData(const QModelIndex& idx, const QVariant& value, int role) override {
        if ( idx.column() == 0 ) {
            auto* propertyValue = static_cast<Property::Value*>(idx.internalPointer());
        ...
        if ( Editor* editor = ::createEditor(*m_form, value, parent, true).release() ) {
            ...
            if ( value->property()->valueType() == Property::Type::Enum ) {
                ...
                connect(editor, &Editor::changed, [this, value, remove] {
                    ...
                    if ( currentVariant != value->children().end() ) {

                        int newCount = ranges::count_if(currentVariant->get()->children(),
                                                        std::not_fn(&Property::isConstant),
                                                        &Property::Value::property);


                        currentVariant->get()->setEnabled(false);
                        ...
                        *remove = false;
                        ...
                        currentVariant->get()->setEnabled(true);
...
public:
    ...
    void openPersistentEditor(const QModelIndex& parent = {})
    {
        ...
        if ( parent.isValid() && parent.column() == 1 && !m_view.isRowHidden(parent.row(), parent.parent()) ) {
            ...
            if ( checked.isValid() && checked.value<Qt::CheckState>() == Qt::Unchecked )
                return;
    ...
    connect(&d->m_model, &QAbstractItemModel::dataChanged, [this](const QModelIndex& tl, const QModelIndex& br, const QList<int>& roles){
        if ( roles.contains(Qt::CheckStateRole) ) {
            for ( int row = tl.row(); row <= br.row(); ++row ) {
                ...
                if ( state.isValid() )
                {
                    if ( state.value<Qt::CheckState>() == Qt::Unchecked )
