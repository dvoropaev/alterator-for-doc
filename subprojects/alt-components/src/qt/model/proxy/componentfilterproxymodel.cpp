...
namespace alt
{
ComponentFilterProxyModel::ComponentFilterProxyModel(const std::shared_ptr<EditionRepository> &editions, QObject *parent)
...
bool ComponentFilterProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
    ...
    {
        const auto &cache = index.data(CustomRoles::CacheRole).value<ModelItem::Cache>();
        if (editionRepository->current() && options.testFlag(NonEdition) && !cache.hasRelationToEdition)
        {
            return false;
        }
