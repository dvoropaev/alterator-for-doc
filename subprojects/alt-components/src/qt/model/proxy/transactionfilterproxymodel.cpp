...
TransactionFilterProxyModel::TransactionFilterProxyModel(const std::shared_ptr<TransactionService> &service,
...
{
    const auto transactionComponents = service->current().components();
    std::unordered_set<std::string> requestedComponents;
    ...
    for (const auto &[name, component] : transactionComponents)
    {
        ...
        {
            service->discard(service->current(), component);
        }
    }
}
...
void TransactionFilterProxyModel::setDisabledBaseComponents(bool value)
{
    ...
    auto it = std::find_if(currentEdition->get().sections.begin(),
                           currentEdition->get().sections.end(),
                           [](const auto &section) { return section.name == "base"; });
    if (it == currentEdition->get().sections.end())
    {
        return;
    }
