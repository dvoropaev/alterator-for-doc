...
namespace alt
{
TransactionTabWidget::TransactionTabWidget(const std::shared_ptr<TransactionService> &service,
...
void TransactionTabWidget::setComponentList(const Transaction &transaction)
{
    ...
    {
        QListWidgetItem *item = new QListWidgetItem(component.get().displayName(lang).data());
        ...
        {
            if (transaction.status(component).isBase && service->safeMode().test(TransactionService::SafeMode::Base))
            {
                item->setIcon(icon);
...
void TransactionTabWidget::setPackageList(const Transaction &transaction)
{
    ...
    {
        QListWidgetItem *item = new QListWidgetItem(name.data());
        if (transaction.action(name) == Transaction::Action::Install)
        {
        ...
        else
        {
            ...
            if (status.isBase && service->safeMode().test(TransactionService::SafeMode::Base))
            {
                ...
                for (const auto &name : status.relation)
                {
                    ...
                    displayNames.push_back(optionalComponent->get().displayName(lang).data());
                }
