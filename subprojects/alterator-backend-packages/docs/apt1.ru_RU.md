...
| [UpdateAsync](#method-UpdateAsync) | Обновляет списки пакетов. |
| [ApplyAsync](#method-ApplyAsync) | Применяет транзакцию установки/удаления. |
...
Создаёт временный файл pkgpriorities (mktemp "${TMPDIR:-/tmp}/alterator-pkgpriorities.XXXXXXXXXXXX"), который перечисляет пакеты, помеченные системой apt как manual командой apt-mark showmanual; предотвращает неявное удаление этих пакетов, если они не указаны в exclude_pkgnames. Файл pkgpriorities используется apt для задания приоритетов (pinning) при принятии решений об удалении. Временный файл удаляется после выполнения метода.
...
Одна строка с датой и временем.
...
Код завершения утилиты определения времени.
