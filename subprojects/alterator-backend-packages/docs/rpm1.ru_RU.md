...
| Метод | Описание |
|--------|---------|
| [Info](#method-Info) | Возвращает статический дескриптор объекта бэкенда rpm. |
| [List](#method-List) | Перечисляет установленные пакеты через rpm -qa с именем, версией, релизом, архитектурой и группой. |
| [Install](#method-Install) | Устанавливает или обновляет файл пакета через rpm -U. |
...
Возвращает статический дескриптор объекта бэкенда rpm.
...
Записи пакетов, отформатированные как «name version release arch group».
...
Устанавливает или обновляет файл пакета через rpm -U.
...
Код завершения перечисления файлов.
