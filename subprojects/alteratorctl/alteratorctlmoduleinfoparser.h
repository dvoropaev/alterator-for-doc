...
#define IS_ALTERATOR_INFO_PARSER(obj) \
    (G_TYPE_CHECK_INSTANCE_TYPE((obj), TYPE_ALTERATOR_MODULE_INFO_PARSER))
...
#define IS_ALTERATOR_INFO_PARSER_CLASS(klass) \
    (G_TYPE_CHECK_CLASS_TYPE((klass), TYPE_ALTERATOR_MODULE_INFO_PARSER))
...
typedef struct
{
    ...
    int (*alterator_ctl_module_info_parser_get_specified_object_data)(gpointer self,
                                                                      gpointer gdbus_source,
                                                                      const gchar* path,
                                                                      const gchar* iface,
                                                                      GNode** result);

    int (*alterator_ctl_module_info_parser_get_data_from_text)(gpointer self, gpointer gdbus_source,
    ...
    GNode* (*alterator_ctl_module_info_parser_get_node_by_name)(gpointer self, GNode* root,
                                                                const gchar* name, guint depth);

    // Depth = -1 -> unlimited depth
    gboolean (*alterator_ctl_module_info_parser_find_table)(gpointer self, GNode* root,
                                                            GHashTable** result, guint depth,
                                                            const gchar* first_table_name, ...);

    gboolean (*alterator_ctl_module_info_parser_find_value)(gpointer self, GNode* root,
                                                            toml_value** result,
