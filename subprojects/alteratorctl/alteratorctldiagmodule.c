...
static int diag_module_parse_options(AlteratorCtlDiagModule* module, int* argc, char** argv,
                                     enum diag_sub_commands command);
static int diag_module_parse_arguments(AlteratorCtlDiagModule* module, int argc, char** argv,
                                       alteratorctl_ctx_t** ctx);

static int diag_module_get_display_name(AlteratorGDBusSource* gdbus_source,
                                        const gchar* diag_str_id, GNode* data, gchar** result);
static int diag_module_get_test_display_name(AlteratorGDBusSource* gdbus_source,
                                             const gchar* tool_path, const gchar* test_name,
                                             gchar** result);
static diag_data_t* diag_module_data_new(AlteratorGDBusSource* gdbus_source,
                                         const gchar* diag_str_id, GNode* diag_node,
                                         diag_module_elem_type type);
...
static int diag_module_list_tests_subcommand(AlteratorCtlDiagModule* module,
                                             alteratorctl_ctx_t** ctx);
static int diag_module_list_all_subcommand(AlteratorCtlDiagModule* module,
                                           alteratorctl_ctx_t** ctx);
static int diag_module_report_subcommand(AlteratorCtlDiagModule* module, alteratorctl_ctx_t** ctx);
static int diag_module_run_tool_test_subcommand(AlteratorCtlDiagModule* module,
                                                alteratorctl_ctx_t** ctx, int* result);
static int diag_module_run_tool_subcommand(AlteratorCtlDiagModule* module, alteratorctl_ctx_t** ctx,
                                           int* result);

static int diag_module_handle_results(AlteratorCtlDiagModule* module, alteratorctl_ctx_t** ctx);
static int diag_module_handle_info_results(AlteratorCtlDiagModule* module,
                                           alteratorctl_ctx_t** ctx);
static int diag_module_handle_list_tests_results(AlteratorCtlDiagModule* module,
                                                 alteratorctl_ctx_t** ctx);
static int diag_module_handle_list_all_results(AlteratorCtlDiagModule* module,
                                               alteratorctl_ctx_t** ctx);
static int diag_module_handle_report_results(AlteratorCtlDiagModule* module,
                                             alteratorctl_ctx_t** ctx);
static int diag_module_handle_tool_test_results(AlteratorCtlDiagModule* module,
                                                alteratorctl_ctx_t** ctx);
static int diag_module_handle_tool_results(AlteratorCtlDiagModule* module,
                                           alteratorctl_ctx_t** ctx);

static int diag_module_print_list_with_filters(AlteratorCtlDiagModule* module,
                                               diag_data_t* diag_data);

static int diag_module_validate_alterator_entry(AlteratorGDBusSource* source,
                                                GNode* alterator_entry_data);

static int diag_module_validate_object_and_iface(AlteratorGDBusSource* source, const gchar* object,
                                                 const gchar* iface);

static int diag_module_get_list_of_tests(AlteratorCtlDiagModule* module,
                                         AlteratorGDBusSource* source, const gchar* path,
                                         GHashTable** result);

static int diag_module_get_and_run_test(AlteratorCtlDiagModule* module,
                                        AlteratorGDBusSource* source, const gchar* diag_tool_str_id,
                                        const gchar* test, DiagRunAdditionalData* run_data,
                                        int* result);

static int diag_module_run_test(AlteratorCtlDiagModule* module, AlteratorGDBusSource* source,
                                const gchar* path, const gchar* test,
                                DiagRunAdditionalData* run_data, int* result);

static int diag_module_get_file_suffix(AlteratorGDBusSource* source, const gchar* path,
                                       gchar** file_suffix);

static int diag_module_get_report(AlteratorGDBusSource* source, const gchar* path,
                                  GVariant** result);
...
GType alterator_ctl_diag_module_get_type(void)
{
    ...
    if (!diag_module_type)
    {
        static const GTypeInfo diag_module_info =
            {sizeof(AlteratorCtlDiagModuleClass),     /* class structure size */
        ...
        const GInterfaceInfo alterator_module_interface_info = {
            ...
            (GInterfaceFinalizeFunc)
                diag_module_alterator_interface_finalize, /* interface_finalize */
            NULL                                          /* interface_data */
        };

        diag_module_type = g_type_register_static(G_TYPE_OBJECT, /* parent class */
                                                  "AlteratorCtlDiagModule", &diag_module_info, 0);

        g_type_add_interface_static(diag_module_type, TYPE_ALTERATOR_CTL_MODULE,
...
alterator_ctl_module_t* get_diag_module()
{
    ...
    {
        gsize module_id_size = g_strlcpy(diag_module.id, ALTERATOR_CTL_DIAG_MODULE_NAME,
...
AlteratorCtlDiagModule* diag_module_new(gpointer app)
{
    ...
    object->gdbus_system_source = alterator_gdbus_source_new(object->alterator_ctl_app->arguments
                                                                 ->verbose,
                                                             G_BUS_TYPE_SYSTEM);

    if (!alterator_ctl_is_root())
        object->gdbus_session_source = alterator_gdbus_source_new(object->alterator_ctl_app
                                                                      ->arguments->verbose,
                                                                  G_BUS_TYPE_SESSION);
...
static void fill_command_hash_table(GHashTable* commands)
{
    for (int i = 0; i < sizeof(diag_module_subcommands_list) / sizeof(diag_module_subcommands_t);
         i++)
        g_hash_table_insert(commands, diag_module_subcommands_list[i].subcommand,
                            &diag_module_subcommands_list[i].ids);
}
                                        ...
                                        const gchar* diag_str_id, GNode* data, gchar** result)
{
    ...
    if (!info_parser->alterator_ctl_module_info_parser_find_table(
            info_parser, data, &display_name, -1, DIAG_ALTERATOR_ENTRY_DISPLAY_NAME_TABLE_NAME,
            NULL))
    {
        g_printerr(_("Can't get display name of diag tool or test %s. Display name data by key %s "
                     "is empty.\n"),
                   diag_str_id, DIAG_ALTERATOR_ENTRY_DISPLAY_NAME_TABLE_NAME);
        ERR_EXIT();
    }
                                             ...
                                             gchar** result)
{
    ...
    if (info_parser->alterator_ctl_module_info_parser_get_specified_object_data(
            info_parser, gdbus_source, tool_path, DIAG_INTERFACE_NAME, &parsed_tool_alterator_entry)
        < 0)
    {
                                         ...
                                         diag_module_elem_type type)
{
    ...
    result->path = g_strdup(
        gdbus_source->alterator_gdbus_source_get_path_by_name(gdbus_source, diag_str_id,
                                                              DIAG_INTERFACE_NAME));
...
static gint diag_module_sort_result(gconstpointer a, gconstpointer b, gpointer user_data)
{
    ...
    else if ((no_display_name || name_only) && !path_only && !display_name_only)
    {
        ...
        second_comparable_data = g_strdup((const gchar*) diag_data_second->name);
             ...
             && (no_display_name || (is_verbose && no_name) || path_only) && !name_only
             && !display_name_only)
    {
                                       ...
                                       alteratorctl_ctx_t** ctx)
{
    ...
    {
        g_printerr(
            _("Parsing of diag module arguments failed. Module interface isn't initialized.\n"));
        ERR_EXIT();
    }
    ...
    case DIAG_INFO:
        if (argc == 4)
            (*ctx) = alteratorctl_ctx_init_diag(selected_subcommand, argv[3], NULL, NULL, NULL,
                                                NULL);
        else
        {
    ...
    case DIAG_LIST:
        if (argc == 3)
            ...
            (*ctx) = alteratorctl_ctx_init_diag(DIAG_LIST_TESTS_PRIV, argv[3], NULL, NULL, NULL,
                                                NULL);
        else
        {
    ...
    case DIAG_REPORT:
        if (argc == 5)
            (*ctx) = alteratorctl_ctx_init_diag(selected_subcommand, argv[3], argv[4], NULL, NULL,
                                                NULL);
        else
        {
    ...
    case DIAG_RUN:
        if (argc == 4)
            (*ctx) = alteratorctl_ctx_init_diag(DIAG_RUN_TOOL_PRIV, argv[3], NULL, NULL, NULL,
                                                NULL);
        else if (argc == 5)
            (*ctx) = alteratorctl_ctx_init_diag(DIAG_RUN_TOOL_TEST_PRIV, argv[3], argv[4], NULL,
                                                NULL, NULL);
        else
        {
    ...
    default:
        ...
        {
            (*ctx) = alteratorctl_ctx_init_diag(DIAG_LIST_TESTS_PRIV, argv[2], NULL, NULL, NULL,
                                                NULL);
        }
        else
        {
...
static int diag_module_info_subcommand(AlteratorCtlDiagModule* module, alteratorctl_ctx_t** ctx)
{
    ...
    if (!diag_tool_str_id)
    {
        g_printerr(_("The call to the diag info method failed. Tool name or path to it's object on "
                     "D-Bus is empty.\n"));
        ERR_EXIT();
    }

    // Object and interface validation
    if (diag_module_validate_object_and_iface(module->gdbus_system_source, diag_tool_str_id,
                                              DIAG_INTERFACE_NAME)
        > 0)
        object_exist_on_system_bus = TRUE;

    if (!is_root
        && diag_module_validate_object_and_iface(module->gdbus_session_source, diag_tool_str_id,
                                                 DIAG_INTERFACE_NAME)
               > 0)
    ...
    if (!object_exist_on_system_bus || !object_exist_on_session_bus)
    {
        ...
        {
            g_printerr(_("Can't find object %s on system or session bus or it has invalid "
                         "interface.\n"),
                       diag_tool_str_id);
            ERR_EXIT();
        }
    ...
    if (object_exist_on_system_bus && (system_bus || both_buses))
    {
        if (module->gdbus_system_source->alterator_gdbus_source_get_text_of_alterator_entry_by_path(
                module->gdbus_system_source, diag_tool_str_id, DIAG_INTERFACE_NAME,
                &info_from_system_bus)
            < 0)
            ERR_EXIT();
    }
    ...
    if (!is_root && object_exist_on_session_bus && (session_bus || both_buses))
    {
        if (module->gdbus_session_source
                ->alterator_gdbus_source_get_text_of_alterator_entry_by_path(
                    module->gdbus_session_source, diag_tool_str_id, DIAG_INTERFACE_NAME,
                    &info_from_session_bus)
            < 0)
            ERR_EXIT();
    }
    ...
    if (info_from_system_bus || info_from_session_bus)
    {
        ...
        if (!is_root && info_from_session_bus)
        {
            ...
            if (common_info)
                common_info = g_strconcat(common_info, session_bus_prefix, info_from_session_bus,
                                          NULL);
            else
                                             ...
                                             alteratorctl_ctx_t** ctx)
{
    ...
    if (!diag_tool_str_id)
    {
        g_printerr(_("The call to the diag list method failed. Tool name or path to it's object on "
                     "D-Bus is empty.\n"));
        ERR_EXIT();
    }
    ...
    if (object_exist_on_system_bus && (system_bus || both_buses))
    {
        if (diag_module_get_list_of_tests(module, module->gdbus_system_source, diag_tool_str_id,
                                          &tests_from_system_bus)
            < 0)
        {
            g_printerr(_("The call to the diag list method failed. Can't get list of tools from "
                         "object %s on system bus.\n"),
                       diag_tool_str_id);
            ERR_EXIT();
        }
    }
    ...
    if (!is_root && object_exist_on_session_bus && (session_bus || both_buses))
    {
        if (diag_module_get_list_of_tests(module, module->gdbus_session_source, diag_tool_str_id,
                                          &tests_from_session_bus)
            < 0)
        {
            g_printerr(_("The call to the diag list method failed. Can't get list of tools from "
                         "object %s on session bus.\n"),
                       diag_tool_str_id);
            ERR_EXIT();
        }
    }

    result_table = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                         (GDestroyNotify) g_hash_table_unref);

    if (tests_from_system_bus)
        g_hash_table_insert(result_table, strdup(DIAG_LIST_TEST_SYSTEM_RESULT_KEY_NAME),
                            g_hash_table_ref(tests_from_system_bus));

    if (tests_from_session_bus)
        g_hash_table_insert(result_table, strdup(DIAG_LIST_TEST_SESSION_RESULT_KEY_NAME),
...
static int diag_module_list_all_subcommand(AlteratorCtlDiagModule* module, alteratorctl_ctx_t** ctx)
{
    ...
    AlteratorCtlModuleInfoParser* session_info_parser = !is_root ? module->gdbus_session_source
                                                                       ->info_parser
                                                                 : NULL;
    ...
    if ((system_bus || both_buses)
        && system_info_parser->alterator_ctl_module_info_parser_create_names_by_dbus_paths_table(
               system_info_parser, module->gdbus_system_source, DIAG_INTERFACE_NAME)
               < 0)
        ERR_EXIT();

    if (!is_root && (session_bus || both_buses)
        && session_info_parser->alterator_ctl_module_info_parser_create_names_by_dbus_paths_table(
               session_info_parser, module->gdbus_session_source, DIAG_INTERFACE_NAME)
               < 0)
        ERR_EXIT();

    if (system_info_parser->names_by_dbus_paths && (system_bus || both_buses))
        g_hash_table_insert(result, strdup(DIAG_LIST_TOOLS_SYSTEM_RESULT_KEY_NAME),
    ...
    if (!is_root && session_info_parser->names_by_dbus_paths && (session_bus || both_buses))
        g_hash_table_insert(result, strdup(DIAG_LIST_TOOLS_SESSION_RESULT_KEY_NAME),
...
static int diag_module_report_subcommand(AlteratorCtlDiagModule* module, alteratorctl_ctx_t** ctx)
{
    ...
    if (!diag_tool_str_id)
    {
        g_printerr(_("The call to the diag report method failed. Tool name or path to it's object "
                     "on D-Bus is empty.\n"));
        ERR_EXIT();
    }
    ...
    if (object_exist_on_system_bus && (system_bus || both_buses))
    {
        if (diag_module_get_report(module->gdbus_system_source, diag_tool_str_id, &system_report)
            < 0)
            ERR_EXIT();
    }
    ...
    if (!is_root && object_exist_on_session_bus && (session_bus || both_buses))
    {
        if (diag_module_get_report(module->gdbus_session_source, diag_tool_str_id, &session_report)
            < 0)
            ERR_EXIT();
    }
                                                ...
                                                alteratorctl_ctx_t** ctx, int* result)
{
    ...
    gboolean is_root                     = alterator_ctl_is_root();
    DiagRunAdditionalData* run_data = (DiagRunAdditionalData*) ((*ctx) ? (*ctx)->additional_data
                                                                       : NULL);
    ...
    if (!diag_tool_str_id)
    {
        g_printerr(_("The call to the diag run method failed. Tool name or path to it's object on "
                     "D-Bus is empty.\n"));
        ERR_EXIT();
    }
    ...
    if (both_buses)
    {
        ...
        if (object_exist_on_system_bus)
        {
            ...
            if (diag_module_get_and_run_test(module, module->gdbus_system_source, diag_tool_str_id,
                                             test, run_data, result)
                < 0)
                ERR_EXIT();
        }
        ...
        if (!is_root && object_exist_on_session_bus)
        {
            ...
            if (diag_module_get_and_run_test(module, module->gdbus_session_source, diag_tool_str_id,
                                             test, run_data, result)
                < 0)
                ERR_EXIT();
        }
...
static int diag_module_handle_info_results(AlteratorCtlDiagModule* module, alteratorctl_ctx_t** ctx)
{
    ...
    {
        g_printerr(
            _("Can't process the result of diag info method. The diag module doesn't exist.\n"));
        ERR_EXIT();
    }
                                                 ...
                                                 alteratorctl_ctx_t** ctx)
{
    ...
    diag_tool_path = diag_tool_str_id[0] != '/'
                         ? g_strdup((gchar*) module->gdbus_system_source
                                        ->alterator_gdbus_source_get_path_by_name(
                                            module->gdbus_system_source, diag_tool_str_id,
                                            DIAG_INTERFACE_NAME))
                         : g_strdup(diag_tool_str_id);

    if (!diag_tool_path && !is_root)
        diag_tool_path = g_strdup(
            (gchar*) module->gdbus_session_source
                ->alterator_gdbus_source_get_path_by_name(module->gdbus_session_source,
                                                          diag_tool_str_id, DIAG_INTERFACE_NAME));
    ...
    {
        g_printerr(
            _("Can't process the result of diag list method. The diag module doesn't exist.\n"));
        ERR_EXIT();
    }

    if (diag_module_validate_object_and_iface(module->gdbus_system_source, diag_tool_path,
                                              DIAG_INTERFACE_NAME)
        > 0)
    {
        ...
        if ((system_bus || both_buses)
            && system_info_parser->alterator_ctl_module_info_parser_get_specified_object_data(
                   system_info_parser, module->gdbus_system_source, diag_tool_path,
                   DIAG_INTERFACE_NAME, &system_diag_tool_object)
                   < 0)
        {
            ERR_EXIT();
        }
    ...
    if (!is_root
        && diag_module_validate_object_and_iface(module->gdbus_session_source, diag_tool_path,
                                                 DIAG_INTERFACE_NAME)
               > 0)
    {
        exist_in_session_bus = TRUE;
        if ((session_bus || both_buses)
            && session_info_parser->alterator_ctl_module_info_parser_get_specified_object_data(
                   session_info_parser, module->gdbus_session_source, diag_tool_path,
                   DIAG_INTERFACE_NAME, &session_diag_tool_object)
                   < 0)
    ...
    if (g_hash_table_contains(result_table, DIAG_LIST_TEST_SYSTEM_RESULT_KEY_NAME))
        system_bus_result_table = g_hash_table_lookup(result_table,
                                                      DIAG_LIST_TEST_SYSTEM_RESULT_KEY_NAME);

    if (g_hash_table_contains(result_table, DIAG_LIST_TEST_SESSION_RESULT_KEY_NAME))
        session_bus_result_table = g_hash_table_lookup(result_table,
                                                       DIAG_LIST_TEST_SESSION_RESULT_KEY_NAME);
    ...
    if (exist_in_system_bus && system_bus_result_table && (system_bus || both_buses))
    {
        ...
        if (!(tests = system_info_parser->alterator_ctl_module_info_parser_get_node_by_name(
                  system_info_parser, system_diag_tool_object,
                  DIAG_ALTERATOR_ENTRY_TESTS_TABLE_NAME, -1)))
        {
        ...
        for (GNode* test = tests->children; test != NULL; test = test->next)
        {
            ...
            diag_data_t* data               = diag_module_data_new(module->gdbus_system_source,
                                                                   diag_data->node_name, test, TEST);
            g_ptr_array_add(system_bus_result, data);
        }

        g_ptr_array_sort_with_data(system_bus_result, diag_module_sort_result,
                                   (gpointer) &module->alterator_ctl_app->arguments->verbose);

        for (guint i = 0; i < system_bus_result->len; i++)
            diag_module_print_list_with_filters(module,
                                                (diag_data_t*) g_ptr_array_index(system_bus_result,
                                                                                 i));
    }
    ...
    if (exist_in_session_bus && session_bus_result_table && (session_bus || both_buses))
    {
        ...
        if (!(tests = session_info_parser->alterator_ctl_module_info_parser_get_node_by_name(
                  session_info_parser, session_diag_tool_object,
                  DIAG_ALTERATOR_ENTRY_TESTS_TABLE_NAME, -1)))
        {
        ...
        g_ptr_array_sort_with_data(session_bus_result, diag_module_sort_result,
                                   (gpointer) &module->alterator_ctl_app->arguments->verbose);

        for (guint i = 0; i < session_bus_result->len; i++)
            diag_module_print_list_with_filters(module,
                                                (diag_data_t*) g_ptr_array_index(session_bus_result,
                                                                                 i));
    }
                                               ...
                                               alteratorctl_ctx_t** ctx)
{
    ...
    system_diag_tools_objects = system_info_parser
                                    ->alterator_ctl_module_info_parser_get_objects_data(
                                        system_info_parser, module->gdbus_system_source,
                                        DIAG_INTERFACE_NAME, &system_diag_tools_amount);

    session_diag_tools_objects = !is_root
                                     ? session_info_parser
                                           ->alterator_ctl_module_info_parser_get_objects_data(
                                               session_info_parser, module->gdbus_session_source,
                                               DIAG_INTERFACE_NAME, &session_diag_tools_amount)
                                     : NULL;
    ...
    if (g_hash_table_contains(result_table, DIAG_LIST_TOOLS_SYSTEM_RESULT_KEY_NAME))
        system_bus_result_table = g_hash_table_lookup(result_table,
                                                      DIAG_LIST_TOOLS_SYSTEM_RESULT_KEY_NAME);

    if (g_hash_table_contains(result_table, DIAG_LIST_TOOLS_SESSION_RESULT_KEY_NAME))
        session_bus_result_table = g_hash_table_lookup(result_table,
                                                       DIAG_LIST_TOOLS_SESSION_RESULT_KEY_NAME);
    ...
    if (system_bus_result_table && (system_bus || both_buses))
    {
        ...
        {
            alterator_entry_node* diag_data = (alterator_entry_node*) system_diag_tools_objects[i]
                                                  ->data;
            toml_value* diag_name = g_hash_table_lookup(diag_data->toml_pairs,
                                                        DIAG_ALTERATOR_ENTRY_TOOLNAME_KEY_NAME);

            diag_data_t* data = diag_module_data_new(module->gdbus_system_source,
                                                     diag_name->str_value,
                                                     system_diag_tools_objects[i], TOOL);
    ...
    if (!is_root && session_bus_result_table && (session_bus || both_buses))
    {
        ...
        {
            alterator_entry_node* diag_data = (alterator_entry_node*) session_diag_tools_objects[i]
                                                  ->data;
            toml_value* diag_name = g_hash_table_lookup(diag_data->toml_pairs,
                                                        DIAG_ALTERATOR_ENTRY_TOOLNAME_KEY_NAME);
            ...
            diag_data_t* data = diag_module_data_new(module->gdbus_session_source,
                                                     diag_name->str_value,
                                                     session_diag_tools_objects[i], TOOL);
    ...
    if (system_diag_tools_objects)
        alterator_ctl_module_info_parser_result_trees_free(system_diag_tools_objects,
                                                           system_diag_tools_amount);

    if (session_diag_tools_objects)
        alterator_ctl_module_info_parser_result_trees_free(session_diag_tools_objects,
                                             ...
                                             alteratorctl_ctx_t** ctx)
{
    ...
    {
        g_printerr(
            _("Can't process the result of diag report method. The diag module doesn't exist.\n"));
        ERR_EXIT();
    }
    ...
    {
        g_printerr(
            _("Can't process the result of diag report method. Report filename is empty.\n"));
        ERR_EXIT();
    }
    ...
    for (gsize i = 0; i < result_array->len; i++)
    {
        ...
        g_variant_get(answer_array, "(ayi)", NULL, NULL);
                                                ...
                                                GNode* alterator_entry_data)
{
    ...
    if (!source->info_parser
             ->alterator_ctl_module_info_parser_find_value(source->info_parser,
                                                           alterator_entry_data, NULL,
                                                           DIAG_ALTERATOR_ENTRY_TOOLNAME_KEY_NAME,
                                                           NULL))
    {
        g_printerr(_("Can't get alterator entry data for validation in module diag: field \"%s\" "
                     "is missing.\n"),
                   DIAG_ALTERATOR_ENTRY_TOOLNAME_KEY_NAME);
        ERR_EXIT();
    }
    ...
    if (!source->info_parser
             ->alterator_ctl_module_info_parser_find_table(source->info_parser,
                                                           alterator_entry_data, NULL, -1,
                                                           DIAG_ALTERATOR_ENTRY_TESTS_TABLE_NAME,
                                                           NULL))
    {
        g_printerr(_("Can't get alterator entry data for validation in module diag: table \"%s\" "
                     "is missing.\n"),
                   DIAG_ALTERATOR_ENTRY_TESTS_TABLE_NAME);
        ERR_EXIT();
    }
...
static int diag_module_validate_object_and_iface(AlteratorGDBusSource* source,
                                                 const gchar* object_str_id, const gchar* iface)
{
    int ret          = 0;
    int object_exist = 0;
    int iface_exists = 0;
    ...
    // Check object
    if (source->alterator_gdbus_source_check_object_by_path(source, object_str_id, &object_exist)
        < 0)
    ...
    if (source->alterator_gdbus_source_check_object_by_iface(source, object_str_id, iface,
                                                             &iface_exists)
        < 0)
        ERR_EXIT();
                                         ...
                                         GHashTable** result)
{
    ...
    d_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, path, DIAG_INTERFACE_NAME, DIAG_LIST_METHOD_NAME,
...
static void diag_module_run_stdout_signal_handler(
...
static void diag_module_run_stderr_signal_handler(
    GDBusConnection* connection, const gchar* sender_name, const gchar* object_path,
    const gchar* interface_name, const gchar* signal_name, GVariant* parameters, gpointer user_data)
{
    for (int i = 0; i < g_variant_n_children(parameters); i++)
                                        ...
                                        int* result)
{
    ...
    gchar* diag_tool_path   = diag_tool_str_id[0] != '/'
                                  ? g_strdup(source->alterator_gdbus_source_get_path_by_name(
                                      source, diag_tool_str_id, DIAG_INTERFACE_NAME))
                                  : g_strdup(diag_tool_str_id);
    int exit_code           = 0;
                                ...
                                DiagRunAdditionalData* run_data, int* result)
{
    ...
    {
        g_printerr(
            _("Running test in diag tool failed. Can't allocate memory for stdout signal.\n"));
        ERR_EXIT();
    }
    ...
    {
        g_printerr(
            _("Running test in diag tool failed. Can't allocate memory for stderr signal.\n"));
        ERR_EXIT();
    }
    ...
    d_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, path, DIAG_INTERFACE_NAME, DIAG_RUN_METHOD_NAME,
                          module->alterator_ctl_app->arguments->verbose);
    if (!d_ctx)
    {
        g_printerr(
            _("Running test in diag tool failed. Can't allocate memory for D-Bus context.\n"));
        ERR_EXIT();
    }
    ...
    text_style status_color = (*result) == PASS || (*result) == SKIP
                                  ? GREEN
                                  : ((*result) == WARN ? YELLOW : RED);
                                       ...
                                       gchar** file_suffix)
{
    ...
    if (source->info_parser->alterator_ctl_module_info_parser_get_specified_object_data(
            source->info_parser, source, path, DIAG_INTERFACE_NAME, &parsed_tool_alterator_entry)
        < 0)
    {
    ...
    if (!source->info_parser
             ->alterator_ctl_module_info_parser_find_value(source->info_parser,
                                                           parsed_tool_alterator_entry, &suffix,
                                  ...
                                  GVariant** result)
{
    ...
    d_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, path, DIAG_INTERFACE_NAME,
                          DIAG_REPORT_METHOD_NAME, source->verbose);
