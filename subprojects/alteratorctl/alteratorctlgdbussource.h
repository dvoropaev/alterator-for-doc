...
#define ALTERATOR_GDBUS_SOURCE(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), TYPE_ALTERATOR_GDBUS_SOURCE, GDBusSource))
#define IS_ALTERATOR_GDBUS_SOURCE(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), TYPE_ALTERATOR_GDBUS_SOURCE))
#define ALTERATOR_GDBUS_SOURCE_CLASS(klass) \
    (G_TYPE_CHECK_CLASS_CAST((klass), TYPE_ALTERATOR_GDBUS_SOURCE, GDBusSourceClass))
#define IS_ALTERATOR_GDBUS_SOURCE_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), TYPE_ALTERATOR_GDBUS_SOURCE))
...
typedef struct
{
    ...
    int (*alterator_gdbus_source_get_object_ifaces)(gpointer self, const gchar *path, GHashTable **ifaces);

    int (*alterator_gdbus_source_get_iface_objects)(gpointer self, const gchar *iface, GHashTable **objects);

    int (*alterator_gdbus_source_get_signals)(
     ...
     *  \returns 0 - if object isn't exists, not null - if object exists, 2 if path contains a path other than path.
     *           This may be the name of an object, no further verification of the object's existence is performed
     */
    int (*alterator_gdbus_source_check_object_by_path)(gpointer self, const gchar *object_str_id, int *result);

    int (*alterator_gdbus_source_check_object_by_iface)(gpointer self,
    ...
    int (*alterator_gdbus_source_get_introspection)(gpointer self,
                                                    const gchar *service,
                                                    const gchar *path,
                                                    gchar **introspection);

    const gchar *(*alterator_gdbus_source_get_name_by_path)(gpointer self, const gchar *path, const gchar *iface);

    const gchar *(*alterator_gdbus_source_get_path_by_name)(gpointer self, const gchar *name, const gchar *iface);
