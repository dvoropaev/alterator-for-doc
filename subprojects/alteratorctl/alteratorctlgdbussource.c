...
static void alterator_gdbus_source_callback(GObject* source_object, GAsyncResult* res,
                                            gpointer user_data);

static void alterator_gdbus_source_on_call_proxy_ready(GObject* source_object, GAsyncResult* res,
                                                       gpointer user_data);
static void alterator_gdbus_source_on_call_get_ready(GObject* source_object, GAsyncResult* res,
                                                     gpointer user_data);
static int alterator_gdbus_source_subscribe_signal(gpointer self, subscribe_signals_t* signal,
                                                   const gchar* interface_name, const gchar* path);
...
static int alterator_gdbus_source_get_object_ifaces(gpointer self, const gchar* path,
                                                    GHashTable** ifaces);

static int alterator_gdbus_source_get_iface_objects(gpointer self, const gchar* iface,
                                                    GHashTable** objects);

static int alterator_gdbus_source_get_signals(gpointer self, const gchar* object_str_id,
...
static int alterator_gdbus_source_check_object_by_path(gpointer self, const gchar* object_str_id,
                                                       int* result);

static int alterator_gdbus_source_check_object_by_iface(gpointer self, const gchar* object_str_id,
...
static int alterator_gdbus_source_getobjets_priv(gpointer* self, const gchar* iface,
                                                 GVariant** result);

static int alterator_gdbus_source_getifaces_priv(gpointer* self, const gchar* path,
                                                 GVariant** result);

static int alterator_gdbus_source_getsignals_priv(gpointer* self, const gchar* path,
                                                  const gchar* interface, const gchar* method,
                                                  GVariant** result);

static int alterator_gdbus_source_get_services_names(gpointer self, GHashTable** result);

static int alterator_gdbus_source_get_introspection(gpointer self, const gchar* service,
                                                    const gchar* path, gchar** introspection);

static const gchar* alterator_gdbus_source_get_name_by_path(gpointer self, const gchar* path,
                                                            const gchar* iface);

static const gchar* alterator_gdbus_source_get_path_by_name(gpointer self, const gchar* name,
                                                            const gchar* iface);
...
AlteratorGDBusSource* alterator_gdbus_source_new(gboolean is_verbose, GBusType type)
{
    ...
    object->alterator_gdbus_source_check_object_by_path =
        &alterator_gdbus_source_check_object_by_path;

    object->alterator_gdbus_source_check_object_by_iface =
        &alterator_gdbus_source_check_object_by_iface;
...
void alterator_gdbus_source_on_manager_bus_get_ready(GObject* source_object, GAsyncResult* res,
                                                     gpointer user_data)
{
    GError* error = NULL;
...
void call(gpointer self, dbus_ctx_t* dbus_ctx, GError** dbus_call_error)
{
    ...
    if (!g_variant_is_object_path(dbus_ctx->path))
    {
        if (!is_object_path(dbus_ctx->path))
        {
            gchar* path = NULL;
            if (!(path = g_strdup(
                      source->alterator_gdbus_source_get_path_by_name(source, dbus_ctx->path,
                                                                      dbus_ctx->interface))))
                return;
    ...
    g_dbus_connection_call(source->dbus_connection, dbus_ctx->service_name, dbus_ctx->path,
                           dbus_ctx->interface, dbus_ctx->method, dbus_ctx->parameters,
                           dbus_ctx->reply_type, G_DBUS_CALL_FLAGS_NONE, dbus_ctx->timeout_msec,
                           NULL, alterator_gdbus_source_callback, user_data);
                       ...
                       GError** dbus_call_error)
{
    ...
    gpointer user_data[2] = {dbus_ctx, dbus_call_error};
    g_dbus_proxy_new(source->dbus_connection, G_DBUS_PROXY_FLAGS_NONE, NULL, dbus_ctx->service_name,
                     dbus_ctx->path, dbus_ctx->interface, NULL,
                     alterator_gdbus_source_on_call_proxy_ready, user_data);
...
int alterator_gdbus_source_subscribe_signal(gpointer self, subscribe_signals_t* signal,
                                            const gchar* interface_name, const gchar* path)
{
    if (!self)
        return -1;
    ...
    gchar* signal_name_conn = concat_signal_name_with_connection_name(
        signal->signal_name, g_dbus_connection_get_unique_name(source->dbus_connection));

    gint subscriber = g_dbus_connection_signal_subscribe(source->dbus_connection, NULL,
                                                         interface_name, signal_name_conn, path,
                                                         NULL, G_DBUS_SIGNAL_FLAGS_NO_MATCH_RULE,
                                                         signal->callback, signal->user_data, NULL);
...
void alterator_gdbus_source_on_call_proxy_ready(GObject* source_object, GAsyncResult* res,
                                                gpointer user_data)
{
    int ret           = 0;
    gpointer* data    = (gpointer*) user_data;
    ...
    g_dbus_proxy_call(proxy, ctx->method, ctx->parameters, G_DBUS_CALL_FLAGS_NONE,
                      ctx->timeout_msec, NULL, alterator_gdbus_source_on_call_get_ready, user_data);
...
void alterator_gdbus_source_on_call_get_ready(GObject* source_object, GAsyncResult* res,
                                              gpointer user_data)
{
    int ret = 0;
                                              ...
                                              GHashTable** signals)
{
    ...
    if (!g_variant_is_object_path(object_str_id))
        ...
        {
            if (!(path = g_strdup(source->alterator_gdbus_source_get_path_by_name(source,
                                                                                  object_str_id,
                                                                                  interface))))
                ERR_EXIT();
        }
                                                        ...
                                                        const gchar* iface, int* result)
{
    ...
    if (!g_variant_is_object_path(object_str_id))
    {
        ...
        {
            gchar* path = NULL;
            if (!(path = (gchar*) source->alterator_gdbus_source_get_path_by_name(source,
                                                                                  object_str_id,
                                                                                  iface)))
                ERR_EXIT();
...
static int alterator_gdbus_source_set_env_value(gpointer self, const gchar* name, gchar* value)
{
    ...
    if (!self)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_set_env_value - "
                     "AlteratorGDBusSource *source is "
                     "NULL.\n"));
        ERR_EXIT();
    }
    ...
    {
        g_printerr(_(
            "Internal error in alterator_gdbus_source_set_env_value - environment variable name is "
            "NULL.\n"));
        ERR_EXIT();
    }
    ...
    if (!value)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_set_env_value - environment "
                     "variable value is "
                     "NULL.\n"));
        ERR_EXIT();
    }

    dbus_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, ALTERATOR_MANAGER_PATH,
                             ALTERATOR_MANAGER_INTERFACE, ALTERATOR_MANAGER_SETENVVALUE_METHOD_NAME,
                             source->verbose);
...
static int alterator_gdbus_source_get_env_value(gpointer self, const gchar* name, gchar** result)
{
    ...
    if (!self)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_get_env_value - "
                     "AlteratorGDBusSource *source is "
                     "NULL.\n"));
        ERR_EXIT();
    }
    ...
    {
        g_printerr(_("Internal error in alterator_gdbus_source_get_env_value - "
                     "AlteratorGDBusSource **result is "
                     "NULL.\n"));
    }
    ...
    {
        g_printerr(_(
            "Internal error in alterator_gdbus_source_get_env_value - environment variable name is "
            "NULL.\n"));
        ERR_EXIT();
    }

    dbus_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, ALTERATOR_MANAGER_PATH,
                             ALTERATOR_MANAGER_INTERFACE, ALTERATOR_MANAGER_GETENVVALUE_METHOD_NAME,
                             source->verbose);
                                                                      ...
                                                                      gchar** alterator_entry)
{
    ...
    if (!is_object_path(object_str_id))
        path = g_strdup(
            source->alterator_gdbus_source_get_path_by_name(self, object_str_id, iface));
    else
    ...
    ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, path, iface, ALTERATOR_ENTRY_GET_METHOD,
                        source->verbose);
                                                 ...
                                                 GVariant** result)
{
    ...
    if (!self)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_getobjets_priv - "
                     "AlteratorCtlEditionsModule *module is "
                     "NULL\n"));
        ERR_EXIT();
    }
    ...
    if (iface)
    {
        dbus_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, ALTERATOR_MANAGER_PATH,
                                 ...
                                 ALTERATOR_MANAGER_GETOBJECT_METHOD_NAME, source->verbose);
    ...
    else
    {
        dbus_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, ALTERATOR_MANAGER_PATH,
                                 ALTERATOR_MANAGER_INTERFACE,
                                 ALTERATOR_MANAGER_GETALLOBJECT_METHOD_NAME, source->verbose);
    ...
    {
        g_printerr(_("DBus error in alterator_gdbus_source_getobjets_priv while calling "
                     "GetObjects() at the object %s: "
    ...
    {
        g_printerr(_("D-Bus error in alterator_gdbus_source_getobjets_priv while calling "
                     "GetObjects(): failed to produce a "
                     "result.\n"));
        ERR_EXIT();
    }
                                                 ...
                                                 GVariant** result)
{
    ...
    if (!self)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_getifaces_priv - "
                     "AlteratorGDBusSource *module is "
                     "NULL.\n"));
        ERR_EXIT();
    }
    ...
    if (path)
    {
        dbus_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, ALTERATOR_MANAGER_PATH,
                                 ALTERATOR_MANAGER_INTERFACE,
                                 ALTERATOR_MANAGER_GETINTERFACES_METHOD_NAME, source->verbose);
    ...
    else
    {
        dbus_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, ALTERATOR_MANAGER_PATH,
                                 ALTERATOR_MANAGER_INTERFACE,
                                 ALTERATOR_MANAGER_GETALLINTERFACES_METHOD_NAME, source->verbose);
    ...
    {
        g_printerr(_("D-Bus error in alterator_gdbus_source_getifaces_priv while calling "
                     "GetObjects() at the object %s: "
                     "failed to produce a result.\n"),
                   path);
        ERR_EXIT();
                                                  ...
                                                  GVariant** result)
{
    ...
    if (!self)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_getsignals_priv - "
                     "AlteratorGDBusSource *module is "
                     "NULL.\n"));
        ERR_EXIT();
    }
    ...
    dbus_ctx = dbus_ctx_init(ALTERATOR_SERVICE_NAME, ALTERATOR_MANAGER_PATH,
                             ALTERATOR_MANAGER_INTERFACE, ALTERATOR_MANAGER_GETSIGNALS_METHOD_NAME,
                             source->verbose);
    ...
    if (!dbus_ctx->result)
    {
        g_printerr(_("D-Bus error in manager module while calling GetSignals() at the object and "
                     "interface %s %s and method "
...
static int alterator_gdbus_source_get_services_names(gpointer self, GHashTable** result)
{
    ...
    if (!self)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_get_services_names - "
                     "AlteratorGDBusSource *module is "
                     "NULL.\n"));
        ERR_EXIT();
    }
    ...
    dbus_ctx = dbus_ctx_init(FREEDESKTOP_DBUS_SERVICE, FREEDESKTOP_DBUS_OBJECT,
                             FREEDESKTOP_DBUS_INTERFACE, FREEDESKTOP_DBUS_GET_SERVICES_METHOD_NAME,
                             source->verbose);
    ...
    if (!dbus_ctx->result)
    {
        g_printerr(_("D-Bus error in while calling ListNames() at service, the object, interface "
                     "%s %s %s and method "
                     "%s: failed to produce a result.\n"),
                   FREEDESKTOP_DBUS_SERVICE, FREEDESKTOP_DBUS_OBJECT, FREEDESKTOP_DBUS_INTERFACE,
                                                    ...
                                                    const gchar* path, gchar** introspection)
{
    ...
    if (!self)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_get_introspection - "
                     "AlteratorGDBusSource *module is "
                     "NULL.\n"));
        ERR_EXIT();
    }
    ...
    dbus_ctx = dbus_ctx_init(service, path, FREEDESKTOP_DBUS_INTROSPECTABLE_INTERFACE,
                             FREEDESKTOP_DBUS_GET_INTROSPECT_METHOD_NAME, source->verbose);
    ...
    if (!dbus_ctx->result)
    {
        g_printerr(_("D-Bus error in while calling Introspect() at service, the object, interface "
                     "%s %s %s and method "
                     "%s: failed to produce a result.\n"),
                   service, path, FREEDESKTOP_DBUS_INTROSPECTABLE_INTERFACE,
                                                            ...
                                                            const gchar* iface)
{
    ...
    if (!source)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_get_name_by_path - "
                     "AlteratorGDBusSource *module is "
                     "NULL.\n"));
        return NULL;
    }
    ...
    if (info_parser->names_by_dbus_paths
        && info_parser
                   ->alterator_ctl_module_info_parser_create_names_by_dbus_paths_table(info_parser,
                                                                                       self, iface)
               < 0)
        return NULL;

    return (const gchar*) g_hash_table_lookup(info_parser->names_by_dbus_paths,
                                                            ...
                                                            const gchar* iface)
{
    ...
    if (!source)
    {
        g_printerr(_("Internal error in alterator_gdbus_source_get_path_by_name - "
                     "AlteratorGDBusSource *module is "
                     "NULL.\n"));
        return NULL;
    }
    ...
    if (info_parser->alterator_ctl_module_info_parser_create_names_by_dbus_paths_table(info_parser,
                                                                                       self, iface)
        < 0)
        return NULL;
...
static gboolean is_object_path(const gchar* str_id) { return str_id[0] == '/'; }
