...
Необработанный вывод помощника systeminfo get-all.
...
Возвращает модель CPU, число логических ядер и частоту через `systeminfo cpu`.

#### Выходные аргументы

##### **stdout_strings** : `as` <a id="argument-stdout_strings-of-GetCPU"></a>

Свойства CPU, возвращаемые помощником.

Элементы stdout_strings: модель; логические ядра; частота в МГц.
...
Строка имени хоста из команды `hostname`.
...
Возвращает суммарный объём накопителей через `systeminfo drive`.

#### Выходные аргументы

##### **stdout_strings** : `as` <a id="argument-stdout_strings-of-GetDrive"></a>

Суммарный объём накопителей из `/proc/partitions`.

Единственный элемент с размером в байтах.
...
Перечисляет VGA-адаптеры через `systeminfo gpu`.

#### Выходные аргументы

##### **stdout_strings** : `as` <a id="argument-stdout_strings-of-GetGPU"></a>

Описания адаптеров, разобранные из вывода `lspci`.
...
Сообщает объём RAM через `systeminfo memory`.

#### Выходные аргументы

##### **stdout_strings** : `as` <a id="argument-stdout_strings-of-GetMemory"></a>

Объём памяти, вычисленный как `MemTotal * 1024`.
...
Возвращает ветку дистрибутива через `systeminfo branch`.
...
Возвращает версию ядра через `systeminfo kernel`.
...
Возвращает человекочитаемое имя операционной системы через `systeminfo os-name`.
...
Возвращает текст лицензии с учётом локали и редакции через `systeminfo os-license`.

#### Выходные аргументы

##### **stdout_bytes** : `ay` <a id="argument-stdout_bytes-of-GetLicense"></a>

Текст лицензии, выбранный помощником.

Порядок поиска: `/usr/lib/alterator/backends/edition` (при наличии `alterator-backend-edition-utils`), `license.<lang>.html` в `/usr/share/alt-notes`, `/usr/share/alt-license`, `/var/lib/install3/licenses`; запасной файл — `license.all.html`. Перед запуском LC_ALL сбрасывается.
...
Возвращает релиз-ноты через `systeminfo release-notes`.

#### Выходные аргументы

##### **stdout_bytes** : `ay` <a id="argument-stdout_bytes-of-GetReleaseNotes"></a>

Текст релиз-нотов, выводимый помощником.

Может быть HTML или текстовым; LC_ALL сбрасывается перед запуском.
...
Возвращает финальные заметки инсталлятора через `systeminfo final-notes`.

#### Выходные аргументы

##### **stdout_bytes** : `ay` <a id="argument-stdout_bytes-of-GetFinalNotes"></a>

Финальные заметки, сформированные помощником.

LC_ALL сбрасывается перед запуском.
...
Возвращает архитектуру системы через `systeminfo arch`.
...
Перечисляет подключённые мониторы и разрешения через `systeminfo monitor`.

#### Выходные аргументы

##### **stdout_strings** : `as` <a id="argument-stdout_strings-of-GetMonitor"></a>

Имена коннекторов и разрешения из `/sys/class/drm/*/modes`.

Элементы в формате `<device>-<connector> <width>x<height>`.
...
Возвращает производителя, модель и версию системной платы через `systeminfo motherboard`.

#### Выходные аргументы

##### **stdout_strings** : `as` <a id="argument-stdout_strings-of-GetMotherboard"></a>

Идентификация платы из DMI.

Элементы stdout_strings: vendor; model; version.
...
Возвращает системную локаль через `systeminfo locale`.

#### Выходные аргументы

##### **stdout_strings** : `as` <a id="argument-stdout_strings-of-GetLocale"></a>

Строка локали из `/etc/locale.conf`.
...
Перечисляет доступные сеансы рабочего стола через `systeminfo list-desktop-environments`.

#### Выходные аргументы

##### **stdout_strings** : `as` <a id="argument-stdout_strings-of-ListDesktopEnvironments"></a>

Идентификаторы сеансов рабочего стола, обнаруженные в `.desktop`-файлах.

stdout_strings может включать записи `CINNAMON`, `GNOME`, `KDE5`, `KDE6`, `MATE`, `XFCE`.
