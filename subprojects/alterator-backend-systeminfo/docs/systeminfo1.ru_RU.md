...
stdout_strings содержит элементы:
1. модель;
2. количество логических ядер;
3. частота в МГц.
Код: 0 — успех, != 0 — ошибка.
#### Выходные аргументы
...
Объединяет `/sys/devices/virtual/dmi/id/board_vendor`, `board_name`, `board_version`. stdout_strings содержит элементы:
1. производитель;
2. модель;
3. версия.
Код: 0 — успех, != 0 — ошибка.
