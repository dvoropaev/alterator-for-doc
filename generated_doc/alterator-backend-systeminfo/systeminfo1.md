...
- Purpose: collect consolidated system data for clients.
...
- Purpose: obtain the operating system name in a human-readable form for display; executes `systeminfo os-name`, reading `PRETTY_NAME` from `/etc/os-release`.
...
- Purpose: return the license text with locale and edition awareness; calls `systeminfo os-license`, which first consults `/usr/lib/alterator/backends/edition` when `alterator-backend-edition-utils` is installed, then searches for `license.<lang>.html` in `/usr/share/alt-notes`, `/usr/share/alt-license`, `/var/lib/install3/licenses`, and falls back to `license.all.html`.
...
- Purpose: retrieve the distribution branch for service use; runs `systeminfo branch`, returning the RPM macro `%_priority_distbranch`.
- Parameters: none; returns `stdout_strings` and `response`.
