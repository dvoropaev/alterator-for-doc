...
| [GetMemory](#method-GetMemory) | Сообщает объём RAM через `systeminfo memory`. |
| [GetBranch](#method-GetBranch) | Возвращает ветку дистрибутива через `systeminfo branch`. |
| [GetKernel](#method-GetKernel) | Возвращает версию ядра через `systeminfo kernel`. |
| [GetOperationSystemName](#method-GetOperationSystemName) | Возвращает человекочитаемое имя ОС через `systeminfo os-name`. |
| [GetLicense](#method-GetLicense) | Возвращает текст лицензии с учётом локали и редакции через `systeminfo os-license`. |
| [GetReleaseNotes](#method-GetReleaseNotes) | Возвращает релиз-ноты, сформированные `systeminfo release-notes`. |
| [GetFinalNotes](#method-GetFinalNotes) | Возвращает финальные заметки инсталлятора через `systeminfo final-notes`. |
| [GetArch](#method-GetArch) | Возвращает архитектуру системы через `systeminfo arch`. |
| [GetMonitor](#method-GetMonitor) | Перечисляет подключённые мониторы и разрешения через `systeminfo monitor`. |
| [GetMotherboard](#method-GetMotherboard) | Возвращает производителя, модель и версию системной платы через `systeminfo motherboard`. |
| [GetLocale](#method-GetLocale) | Возвращает системную локаль через `systeminfo locale`. |
| [ListDesktopEnvironments](#method-ListDesktopEnvironments) | Перечисляет доступные сеансы рабочего стола через `systeminfo list-desktop-environments`. |
...
Возвращает имя хоста через `systeminfo host-name`.
