...
void ModelBuilder::setEditionRelationshipForAllComponents()
{
    ...
    const auto &sections = alt::Model::current_edition->sections;

    for (const auto &section : sections)
    {
        for (const QString &componentName : section.components)
        {
            auto componentOpt = ComponentRepository::get<Component>(componentName);
            if (componentOpt.has_value())
            {
                Component &component = componentOpt.value().get();
                component.hasRelationToEdition = true;
            }
        }
    }
}
