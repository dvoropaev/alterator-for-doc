...
namespace alt
{
MainWindow::MainWindow(Controller *controller, Model *model, ProxyModel *proxyModel, QWidget *parent)
    : QMainWindow(parent)
    ...
    , proxyModel(proxyModel)
{
    ...
    this->ui->componentsWidget->setController(this->controller);
...
void MainWindow::setupTranslator()
{
    const auto installTranslator =
        [this](const QString &id, auto &translator, const QString &lang, const QString &path) {
            ...
            if (!translator->load(lang, path))
            {
                ...
                controller->issueMessage(QtMsgType::QtInfoMsg, message);
            }
...
{
    this->controller->itemChanged(dynamic_cast<ModelItem *>(item));
}
...
{
    controller->showContent(isChecked);
}

void MainWindow::on_actionBySections_triggered()
{
    controller->setViewMode(ViewMode::BySections);
}

void MainWindow::on_actionPlain_triggered()
{
    controller->setViewMode(ViewMode::Plain);
}

void MainWindow::on_actionByTags_triggered()
{
    controller->setViewMode(ViewMode::ByTags);
}
...
{
    controller->setNameViewMode(NameViewMode::NamesAndIDs);
}

void MainWindow::on_actionNames_only_triggered()
{
    controller->setNameViewMode(NameViewMode::NamesOnly);
}

void MainWindow::on_actionIDs_only_triggered()
{
    controller->setNameViewMode(NameViewMode::IDsOnly);
}
...
void MainWindow::onLanguageChanged(QAction *action)
{
    ...
    this->controller->changeLocale(locale);
}
